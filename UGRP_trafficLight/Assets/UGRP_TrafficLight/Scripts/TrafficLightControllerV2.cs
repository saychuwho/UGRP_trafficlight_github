...
public class TrafficLightControllerV2 : MonoBehaviour
{
    ...
    public new_york_carLight[] n_trafficLights;
    ...
    void NYCarLightInit()
    {
        // trafficlight의 상태를 init : NY version
        int temp_index = 0;
        foreach (new_york_carLight trafficlight in n_trafficLights)
        {
            // trafficlight reset
            trafficlight.ResetLight();
            ...
            /*
            // add this code when traffic sign is important
            // straight_left_together sign init
            if (trafficlight.trafficLightType / 100 != 2 && trafficlight.trafficLightType / 100 != 5)
            {
                trafficlight.trafficSign.SetActive(false);
            ...
            */
            ...
            // left light, straight light lightState init
            LightStatesGeneratorV2 temp_light_2 = new LightStatesGeneratorV2(500);
            trafficlight.left_light.lightStates = temp_light_2.lightStates;
            trafficlight.straight_light.lightStates = temp_light_2.lightStates;

            // Location init
            trafficlight.trafficLightLocation = temp_index;

            /*
            // add this code when child light init is needed
            // child light init
            if (trafficlight.carChildLights.Length > 0)
            {
                foreach (car_light_UGRP childlight in trafficlight.carChildLights)
            ...
            */
            ...
            /*
            // add this code when child light is needed
            // child light current light init, light on the first state
            if (trafficlight.carChildLights.Length > 0)
            {
                foreach (car_light_UGRP childlight in trafficlight.carChildLights)
    ...
    /// New York Rule will use the same rule as KR Rule when Initialize and Update ped light
    /// </summary>
        ...
        if (pedestrainLights.Length > 0)
        {
            foreach (ped_light_UGRP pedlight in pedestrainLights)
            {
                ...
                if ((pedlight.trafficLightType % 100) / 10 == 1)
                {
                    ...
                    // front traffic light, side traffic light init;
                    int tmp_pedlight_location = (pedlight.trafficLightLocation + 2) % 4;
                    pedlight.pedFrontTrafficlight = (tmp_pedlight_location + 2) % 4;
                    pedlight.pedSideTrafficlight01 = (tmp_pedlight_location + 1) % 4;
                    pedlight.pedSideTrafficlight02 = (tmp_pedlight_location + 3) % 4;
                    pedlight.pedThisTrafficlight = tmp_pedlight_location;
            ...
            if (rightTurnLights.Length > 0)
            {
                foreach (rightTurn_light_UGRP rightlight in rightTurnLights)
                {
                    ...
                    // Location init
                    rightlight.trafficLightLocation = (temp_index + 2) % 4;
                    ...
                    if (rightlight.isRightlightActive)
                    {
                        ...
                        // front traffic light, side ped light init
                        int tmp_right_location = (rightlight.trafficLightLocation + 2) % 4;
                        rightlight.rightFrontTrafficlight = (tmp_right_location + 2) % 4;
                        rightlight.rightSidePedlight = (tmp_right_location + 1) % 4;
    ...
    /// NY Car Light Update
    /// </summary>
    void NYUpdateCarLight()
    {
        // 차량 신호등을 update
        foreach (new_york_carLight trafficlight in n_trafficLights)
        {
            if (trafficlight.left_light != null && trafficlight.straight_light != null)
            {
                if (trafficlight.currentLightStateDuration <= Time.time)
                {
                    trafficlight.currentLightStateIndex++;
    ...
    /// NY traffic rule is using this function too.
    /// </summary>
    ...
    void NYUpdatePedRightLight()
    {
        if (pedestrainLights.Length > 0)
        {
            ...
            foreach (ped_light_UGRP pedlight in pedestrainLights)
            {
                ...
                if (pedlight.lights.Length > 1)
                {
                    new_york_carLight tempFrontLight = n_trafficLights[pedlight.pedFrontTrafficlight];
                    new_york_carLight tempSideLight01 = n_trafficLights[pedlight.pedSideTrafficlight01];
                    new_york_carLight tempSideLight02 = n_trafficLights[pedlight.pedSideTrafficlight02];
                    new_york_carLight tempThisLight = n_trafficLights[pedlight.pedThisTrafficlight];
                    rightTurn_light_UGRP tempRightlight = rightTurnLights[pedlight.pedControlRightlight];
                    ...
                    if (tempRightlight.lights.Length > 1 && tempRightlight.isRightlightActive)
                    {
                        new_york_carLight tempFrontTrafficlight = n_trafficLights[tempRightlight.rightFrontTrafficlight];
                        new_york_carLight tempThisTrafficlight = n_trafficLights[tempRightlight.trafficLightLocation];
